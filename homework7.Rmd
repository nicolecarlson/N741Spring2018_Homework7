...
fitcesd <- rpart(cesd ~ mcs, data=h1)
printcp(fitcesd) # Display the results
plotcp(fitcesd) # Visualize cross-validation results
...
glm1 <- glm(cesd_gte16 ~ age + female + pss_fr + homeless + pcs + mcs, data = h1)
glm1
summary(glm1)

fitk <- rpart(cesd_gte16 ~ age + female + pss_fr + homeless + pcs + mcs, method = "class", data = h1)
class(fitk)
# Display the results
printcp(fitk)
...
summary(fitk)
# Plot the tree
plot(fitk, uniform = TRUE, main = "Classification Tree for CESD => 16")
...
fitallpk <- ctree(cesd_gte16 ~ ., data = h1b)
class(fitallpk)
plot(fitallpk, main = "Conditional Inference Tree for CESD => 16")

# Recursive partitioning of CESD on age, female, pss_fr, homeless, pcs, mcs
whoIsDepressed <- rpart(cesd_gte16 ~ age + female + pss_fr + homeless + pcs + mcs, data = h1b, control = rpart.control(cp = 0.001, minbucket = 20))
#whoIsDepressed <- rpart(cesd_gte16 ~ age + female #+ pss_fr + homeless + pcs + mcs, data = h1b)
whoIsDepressed
# Plot the tree
plot(as.party(whoIsDepressed))
...
fitallrf <- rfsrc(cesd ~ ., data = h1a, ntree = 100, tree.err=TRUE)
# view the results
fitallrf
...
varsel_cesd <- var.select(fitallrf)
glimpse(varsel_cesd)
# Save the gg_minimal_depth object for later use
gg_md <- gg_minimal_depth(varsel_cesd)
# Plot the object
plot(gg_md)
